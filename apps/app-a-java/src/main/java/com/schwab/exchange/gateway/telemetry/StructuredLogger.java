...
public class StructuredLogger {
  ...
  public void dependencyProbeSuccess(TraceContext traceContext, DownstreamResult result) {
    emit(createDependencyProbeSuccessEvent(traceContext, result));
  }

  StructuredLogEvent createDependencyProbeSuccessEvent(
      TraceContext traceContext, DownstreamResult result) {
    return event(
        "INFO",
        "dependency exchange-rate probe succeeded",
        "dependency_probe",
        traceContext,
  ...
  public void dependencyProbeFailure(
    ...
    emit(
        event(
            "WARNING",
            "dependency exchange-rate probe failed",
            "dependency_probe",
            traceContext,
            "/internal/exchange-rates",
            "GET",
  ...
  StructuredLogEvent createRequestEvent(
    ...
    String message = statusCode < 400 ? "exchange rates returned" : "exchange-rate request failed";
    return event(
        severity,
        message,
        "request",
