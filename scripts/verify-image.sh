...
output_format="${VERIFY_IMAGE_FORMAT:-human}"

case "$output_format" in
  human | digest) ;;
  *)
    printf 'VERIFY_IMAGE_FORMAT must be human or digest.\n' >&2
    exit 2
    ;;
esac
...
git_sha="${2:-$(git rev-parse --verify HEAD^{commit})}"
[[ "$git_sha" =~ ^[0-9a-f]{40}$ ]] || {
...
image_path="$repository/$image_name"
inventory="$(
  gcloud --configuration="$GCLOUD_CONFIGURATION" \
    --account="$expected_account" \
    --project="$PROJECT_ID" \
...
if [[ "$output_format" == "digest" ]]; then
  printf '%s\n' "$digest"
else
  printf '%s:%s -> %s\n' "$image_path" "$git_sha" "$digest"
  printf 'Verified immutable full-SHA image tag for %s.\n' "$image_name"
fi
